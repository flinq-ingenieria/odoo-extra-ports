...
class TestIrMailServer(TransactionCase):
    ...
    def test_send_email_change_smtp_server(self):
        #Changes in smtp_from_rfc2822 function, that makes additional checks on address structure 
        #makes MagicMock unsuitable to test this functionality
        ...
        #if not self.env["ir.module.module"].search([("name", "=", "mail_outbound_static"), ("state", "=", "installed")]):
        #    self.assertEqual(mail_server_id, None)

    def test_message_thread_send(self):
        #CHANGES IN with_user makes the test fail while module works correctly
        mail_message_1 = self.partner.with_user(self.user1.id).message_post(body=_("Test"), subtype_xmlid="mail.mt_comment")
