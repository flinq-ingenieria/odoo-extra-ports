...
class AccountInvoiceDownloadCredentials(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        res.update({"download_config_id": config.id, "login": config.login})
        return res

    @api.model
    def prepare_and_remove_credentials(self, vals):
        credentials = {"login": vals.get("login"), "password": vals.get("password")}
    ...
    def run(self):
        ...
        if self.invoice_ids_str:
            ...
            action = self.env["ir.actions.act_window"]._for_xml_id(xmlid)
            action.update(
                {"res_id": self.log_id.id, "views": False, "view_mode": "form,tree"}
            )
        return action
